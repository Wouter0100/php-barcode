...
class EAN13 extends Barcode {
    ...
    public function __construct($number, $scale) {
        if (strlen($number) < 12 || strlen($number) >= 13) {
            throw new Exception('There must be 12 numbers.');
        }
        ...
        $this->_checksum = $this->_checksum($number);
    ...
    /**
     * The following function creates the checksum which will
     * being appended to the 12 numbers.
     */
    protected function _checksum($ean) {
        $ean = (string)$ean;
        $even = true;
        $esum = 0;
        $osum = 0;
    ...
    /**
     * Save's the image to a specific path.
     *
     * Destroy the image afterwards because we're firing and forgetting
     */
